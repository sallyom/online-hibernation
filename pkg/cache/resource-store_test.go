...
func TestProjPodCacheIsInitiallyPopulated(t *testing.T) {
	c := make(chan struct{})
	...
	projr := kcache.NewReflector(projLW, &corev1.Namespace{}, store, 0)
	go projr.Run(c)
	...
	projs, err := store.ByIndex("getProject", "somens1")
	if err != nil {
	...
	pods, err = store.ByIndex("byNamespaceAndKind", "somens3/"+PodKind)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	pods, err = store.ByIndex("byNamespaceAndKind", "somens2/"+PodKind)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	projs, err = store.ByIndex("getProject", "somens5")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	assert.Len(t, projs, 1, "expected to have one namespace called somens5")

	// pods in somens5 is not Phase PodRunning, so it should not be added to cache
	pods, err = store.ByIndex("byNamespaceAndKind", "somens5/"+PodKind)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
